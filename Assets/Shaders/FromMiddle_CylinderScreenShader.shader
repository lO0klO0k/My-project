Shader "Unlit/FromMiddle_CylinderScreenShader"
{
    Properties
    {
        ...
        _MainTex1 ("Texture", 2D) = "while" {}
        _MainTex2 ("Texture", 2D) = "while" {}
        _EndOfScreenAngle("ScreenAngle", Range(0.1, 180)) = 90
        _EndOfScreen("ScreenEnd", Vector) = (-1,1,0,0)
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                if(o.PosLS.g >= 0)
                {
                    theta = 0.5 + 0.5*acos(dot(LS, float3(StartScreenDir, 0))) / TotalAngle;
                
                    z = (o.PosLS.b  + _ScreenHeight/2.f) / _ScreenHeight;
                }
                else
                {
                    theta = 0.5 - 0.5*acos(dot(LS, float3(1, 0, 0))) / TotalAngle;
